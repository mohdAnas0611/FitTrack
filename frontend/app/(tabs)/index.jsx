// app/(tabs)/index.jsx  ←  Home Screen with Dynamic Challenges & Leaderboard
import { useState, useEffect } from "react";
import { AVATAR_CONFIGS, LEADERBOARD as STATIC_LEADERBOARD, XP_PER_LEVEL } from '../../constants';
...
export default function HomeScreen({ onNav }) {
  ...
  const completeChallenge = async (ch) => {
    ...
    if (getToken()) {
      try {
        await api.challenges.claimReward(ch.id);
      } catch (e) {}
    }
  };
    ...
    <div className="screen slide-in" style={{ background: dark ? '#0F0F13' : 'var(--bg)' }}>
      ...
      <div style={{ padding: '0 20px 16px', paddingTop: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 17, fontWeight: 800, color: dark ? '#F0F0F5' : 'var(--charcoal)' }}>{user?.name || 'Player'}</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: dark ? 'rgba(245,158,11,0.15)' : 'var(--gold-light)', padding: '6px 12px', borderRadius: 'var(--r-full)' }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--coin)' }}>{(user?.coins || 0).toLocaleString()}</span>
            </div>
            <div style={{ background: 'var(--orange)', color: '#fff', borderRadius: 'var(--r-full)', padding: '5px 10px', fontSize: 11, fontWeight: 700 }}>Lv.{user?.level || 1}</div>
          </div>
        </div>
        <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--gray)', whiteSpace: 'nowrap' }}>{user?.xp || 0}/{XP_PER_LEVEL} XP</span>
        </div>
      </div>
      ...
      <div style={{ margin: '14px 20px', display: 'flex', gap: 8 }}>
        {[
          { label:'Calories', val:`${user?.calories || 482}`,                         icon:'🔥', color:'var(--orange)' },
          { label:'Active',   val:'48m',                                              icon:'⏱️', color:'var(--purple)' },
          { label:'Streak',   val:`${user?.streak || 1}d`,                            icon:'🔥', color:'var(--gold)'   },
          { label:'Territory',val:`${Math.round(((user?.zones || 0) / 442) * 100)}%`,  icon:'🗺️', color:'var(--green)'  },
        ].map((s, i) => (
          ...
          {leaderboard.slice(0, 4).map((p, i) => {
            ...
            return (
              <div key={i} style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, background: p.isUser ? (dark ? 'rgba(244,98,31,0.12)' : 'rgba(244,98,31,0.06)') : undefined, borderLeft: p.isUser ? '3px solid #F4621F' : undefined, borderBottom: i < 3 ? `1px solid ${dark ? '#2A2A35' : '#E5E7EB'}` : undefined }}>
                <span style={{ fontSize: 18, width: 28 }}>{['🥇','🥈','🥉','4️⃣'][i] || `${i + 1}️⃣`}</span>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--gray)' }}>{p.zones} zones • {((p.steps || 0) / 1000).toFixed(1)}k steps</div>
                </div>

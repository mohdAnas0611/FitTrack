// app/(tabs)/profile.jsx  ←  Profile Screen with Leaderboard & Settings Sync
import { useState, useEffect } from "react";
import { useApp } from '../../hooks';
import { AVATAR_CONFIGS, BADGES, LEADERBOARD as STATIC_LEADERBOARD, XP_PER_LEVEL } from '../../constants';
import { MAP_ROWS, MAP_COLS } from '../../constants/data';
import { api } from '../../services/api';
import { removeToken, getToken } from '../../constants/storage';
...
export default function ProfileScreen({ onLogout }) {
  ...
  const xpPct      = Math.min(100, ((user?.xp || 0) / XP_PER_LEVEL) * 100);
  ...
  const toggleSetting = async (key) => {
    const currentVal = !!(user?.settings || {})[key];
    const nextVal = !currentVal;
    const newSettings = { ...(user?.settings || {}), [key]: nextVal };

    setUser(u => ({ ...u, settings: newSettings }));
    showToast(`${key.charAt(0).toUpperCase() + key.slice(1)} ${nextVal ? 'enabled' : 'disabled'}`);

    if (getToken()) {
      try {
        await api.user.updateProfile({ settings: newSettings });
      } catch (e) {}
    }
  };
    ...
    <div className="screen slide-in" style={{ background: dark ? '#0F0F13' : 'var(--bg)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,var(--orange),var(--orange-hover))', padding: '20px 20px 50px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, position: 'relative' }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: '#fff' }}>{user?.name || 'Player'}</div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>@{user?.username || 'player'}</div>
            <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
              <div style={{ background: 'rgba(255,255,255,0.2)', borderRadius: 'var(--r-full)', padding: '3px 10px', fontSize: 11, fontWeight: 700, color: '#fff' }}>⭐ Level {user?.level || 1}</div>
              <div style={{ background: 'rgba(255,255,255,0.2)', borderRadius: 'var(--r-full)', padding: '3px 10px', fontSize: 11, fontWeight: 700, color: '#fff' }}>🔥 {user?.streak || 1}d streak</div>
            </div>
          </div>
        </div>
        <div style={{ marginTop: 16, position: 'relative' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.8)', fontWeight: 600 }}>{user?.xp || 0}/{XP_PER_LEVEL}</span>
          </div>
      ...
      <div style={{ padding: '0 20px', marginTop: -24, position: 'relative', zIndex: 2 }}>
        ...
        {tab === 'overview' && (
          <>
            ...
            <button className="btn" style={{ width: '100%', padding: '14px', borderRadius: 'var(--r-full)', background: dark ? 'rgba(239,68,68,0.15)' : 'var(--red-light)', color: 'var(--red)', fontSize: 15, fontWeight: 700, fontFamily: 'var(--font)', marginBottom: 100 }} onClick={handleLogout}>🚪 Log Out</button>
          </>
        )}
          ...
          <div style={{ marginBottom: 100 }}>
            <div className="card" style={{ borderRadius: 16, overflow: 'hidden' }}>
              ...
              {leaderboard.map((p, i) => {
                ...
                return (
                  <div key={i} style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, background: p.isUser ? (dark ? 'rgba(244,98,31,0.1)' : 'rgba(244,98,31,0.05)') : undefined, borderLeft: p.isUser ? '3px solid #F4621F' : undefined, borderBottom: i < leaderboard.length - 1 ? `1px solid ${dark ? '#2A2A35' : '#E5E7EB'}` : undefined }}>
                    <span style={{ fontSize: 22, width: 32 }}>{['🥇','🥈','🥉','4️⃣'][i] || `${i + 1}️⃣`}</span>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--gray)', marginTop: 1 }}>{pcfg.name} • {p.zones} zones • {((p.steps || 0) / 1000).toFixed(1)}k steps</div>
                    </div>
